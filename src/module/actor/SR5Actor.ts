...
import { ValueSources, ValueSourcesFlow } from '../data/flows/ValueSourcesFlow';
...
export class SR5Actor extends Actor {
    ...
    getSkill(id: string, options: SkillOptions = { byLabel: false }): Shadowrun.SkillField | undefined {
        if (options.byLabel) return this.getSkillByLabel(id, options);
    ...
    asVehicle(): Shadowrun.VehicleActorData | undefined {
        if (this.isVehicle()) return this.data as Shadowrun.VehicleActorData;
    }

    //@ts-ignore // TODO: foundry-vtt-types v10 - will return the item
    asCharacter(): Shadowrun.CharacterActorData | undefined {
        if (this.isCharacter()) return this.data as Shadowrun.CharacterActorData;
    }
    ...
    /**
     * Add a fire mode's recoil to the progressive recoil.
     *
     * @param fireMode Ranged Weapon fire mode used to attack with.
     */
    ...
    /**
     * When accessing vehicle data with alternative values sources.
     * 
     * In general you should use the general valueSources getter.
     */
    get _vehicleValueSources(): ValueSources {
        if (!this.isVehicle()) throw new Error(`Actor ${this.name} (${this.id}) is not a vehicle`);
        ...
        switch (vehicle.system.controlMode) {
            ...
            default:
                throw new Error(`Unknown control mode ${vehicle.system.controlMode} for vehicle ${this.name} (${this.id})`);
        }
    }

    /**
     * Apply Shadowrun5e rules around altering actor values for testing.
     *
    ...
    getRollData(): Shadowrun.ShadowrunActorDataData {
        ...
        ValueSourcesFlow.applyValueSources<Shadowrun.ShadowrunActorDataData>(rollData, this.valueSources, this);
